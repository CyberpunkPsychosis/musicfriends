...
> **状态更新（2026-06）：Lua 编曲层（§6 的核心 spike）已冻结。**
> 复检发现：Ardour 9.5 官方 MCP 落地但仅无障碍向、不做编曲；Ableton/REAPER 社区
> MCP 已能建轨写音符。「对话驱动 DAW」改走现成生态（见 [daw-mcp-setup.md](daw-mcp-setup.md)、
> [research-2026-06.md](research-2026-06.md)）。本文 OSC 部分仍有效；其余作为存档保留。
>
> 原状态：设计阶段（蓝图）。本会话环境为临时容器，决策落仓库以便跨会话延续。
> 目标读者：未来的你 + 未来的 AI 会话。
